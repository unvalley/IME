...
```sh
IME_BENCH_ITERATIONS=100 IME_BENCH_LIVE_LENGTHS=10 cargo bench -p ime-core --bench engine
```

Live Conversionを省き、分野別辞書の差だけを測る場合:

```sh
IME_BENCH_ITERATIONS=10000 IME_BENCH_LIVE_LENGTHS= cargo bench -p ime-core --bench engine
```
