...
use ime_core::{ALL_DOMAIN_DICTIONARIES, EnginePreferences, ImeEngine, InputEvent, UserData};
...
fn main() {
    ...
    let mut all_packs_engine = ImeEngine::bundled();
...
fn run_session_context_benchmarks(iterations: u64) {
    ...
    let mut empty_context = ImeEngine::bundled();
    ...
    let mut full_context = ImeEngine::bundled();
...
fn commit_reading(engine: &mut ImeEngine, reading: &str, surface: &str) {
...
fn query_and_clear(engine: &mut ImeEngine, reading: &str) {
    for character in reading.chars() {
        black_box(engine.handle(InputEvent::Character(character)));
    }
    black_box(engine.handle(InputEvent::Space));
...
fn run_history_benchmarks(iterations: u64) {
    let directory = std::env::temp_dir().join(format!(
        "unvalley-ime-history-benchmark-{}",
        std::process::id()
    ));
    fs::create_dir_all(&directory).expect("create history benchmark directory");
    let mut fixture = String::from("# unvalley-ime-history-v1\n");
    for index in 0..499 {
        writeln!(
            fixture,
...
fn live_lengths() -> Vec<usize> {
    std::env::var("IME_BENCH_LIVE_LENGTHS").map_or_else(
        |_| vec![10, 50, 100],
        |value| {
            value
                .split(',')
